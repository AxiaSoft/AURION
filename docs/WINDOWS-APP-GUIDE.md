...
```
AURION/
├── apps/desktop/          # Electron app (Windows app اصلی)
│   ├── package.json
...
5. **نصب وابستگی‌های پروژه**:
   - `npm ci` در `backend/` و `apps/desktop/`
   - `pip install -r engine/requirements.txt`
6. **اجرای AURION**: بعد از اتمام، `apps/desktop` را اجرا کند.

### 4.2) نمونه ساده Bootstrapper با Electron (پیشنهادی)

`apps/desktop/src/bootstrapper.js` (pseudo):
...
[Files]
Source: "..\apps\desktop\dist\*"; DestDir: "{app}"; Flags: recursesubdirs
...
- [ ] `apps/desktop` → `npm run dist:win` بدون خطا؟
