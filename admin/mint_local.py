...
try:
    ...
    print("Fix: run from AURION root, e.g.: python admin/mint_local.py developer", file=sys.stderr)
    print("Or set PYTHONPATH: set PYTHONPATH=D:\\AURION BETA\\engine", file=sys.stderr)
    sys.exit(4)

def main():
    if len(sys.argv) < 2:
        print("Usage: python admin/mint_local.py <plan> [note]", file=sys.stderr)
        print(f"Plans: {', '.join(k for k in PLANS if k not in ('freemium','trial'))}", file=sys.stderr)
        print("Example: python admin/mint_local.py developer \"admin-owner\"", file=sys.stderr)
        print("Example: python admin/mint_local.py m1 \"client@example.com\"", file=sys.stderr)
        sys.exit(2)
